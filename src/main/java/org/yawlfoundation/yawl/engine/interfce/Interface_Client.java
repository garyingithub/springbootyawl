...
public class Interface_Client {
    ...
    public Map<String,String> caseEngineSessionMap=new ConcurrentHashMap();


    public Map<String,String> caseEngineMap=new ConcurrentHashMap();


    private String getURI(String urlStr,Map<String,String> paramsMap) throws IOException {
        if(paramsMap.containsKey("workItemID"))
        {
            String workItemID=paramsMap.get("workItemID");
            String caseId=workItemID.split(":")[0];
            String _engineURIStr="";
            if(caseId.contains(".")){
                caseId=caseId.split("\\.")[0];
            }

            _engineURIStr=this.caseEngineMap.get(caseId);


            if(_engineURIStr==null){
                throw new IOException("need the engine address");
            }
            paramsMap.put("sessionHandle",this.caseEngineSessionMap.get(caseId));
            return _engineURIStr;
        }
        else return urlStr;
    }
    /**
    ...
    public String executePost(String urlStr, Map<String, String> paramsMap)
        ...
        urlStr=getURI(urlStr,paramsMap);
        if(paramsMap.containsKey("action")&&
                paramsMap.get("action").equals("launchCase")){
            String result=send(urlStr, paramsMap, true);
            String[] results=result.split("~");
            if(!result.contains("failure")){
                // format caseID~engineURL~sessionHandle
                this.caseEngineMap.put(results[0],results[1]);
                this.caseEngineSessionMap.put(results[0],results[2]);
    ...
    public String originalExecutePost(String urlStr, Map<String, String> paramsMap)
            throws IOException {
