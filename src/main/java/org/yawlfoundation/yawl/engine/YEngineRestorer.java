...
public class YEngineRestorer {
    ...
    protected YCaseNbrStore restoreNextAvailableCaseNumber() throws YPersistenceException {
        ...
        if ((query != null) && (!query.list().isEmpty())) {
            ...
            query = _pmgr.createQuery("select max(rootNetID) from YLogSpecification");
            if ((query != null) && (!query.list().isEmpty())) {

                try {
                    String engineID = query.iterate().next().toString();
    ...
    private void removeOrphanedIdentifiers() {
        ...
        try {
            Query query = _pmgr.createQuery("from org.yawlfoundation.yawl.elements.state.YIdentifier");
            for (Iterator it = query.iterate(); it.hasNext(); ) {
